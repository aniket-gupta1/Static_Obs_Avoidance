...
def no_add_points(sn,dn,obs,dist):
    rad=obs[2]
...
def process(sn,dn,pn):                                        # form [row][col][alt]
    ...
    add=no_add_points(sn,dn,obstacle,dist)  #  to add angle
...
fence_gps=geofencedata("geofence2912.fen")              # array containing gps coordinates [lat][lon] of geofence
...
obstacle_list=plot_obstacle("obstacle2912.txt")                 # list of obstacles in [row][column] format
print "obstacles list  =",obstacle_list
print "\n"
#-- obstacles plotted

waypoint_array,waypoint_gps=plot_waypoint("newgrid2912.waypoints") # list of waypoints in [row][col] or [lat][lon] format respectively
...
file_new=open("newwaypoints1.txt","w")
file_new.write("QGC WPL 110\n")        
for i in range(len(final_path)):
     latt,lonn=grid2gps(final_path[i][0],final_path[i][1])
     string = str(i)+"    0   3   16  0.000000    0.000000    0.000000    0.000000    "+str(latt)+"    "+str(lonn)+"    "+"65.0"+"       1"
     file_new.write(string+"\n")
print "edited in newwaypoints.txt"     
file_new.close()
