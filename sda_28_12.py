...
def plot_obstacle(obstacle_file):
    ...
    for i in lines:
        ...
        l.append(row)                                                 
        l.append(col)
        radius=ft2me(float(thisline[2]))+5
...
def identify_obstacle(snode,dnode,pn):      # form [row][col][alt]
    ...
    # obstacle list after slope sorting
    # sort distance       
    for i in range (len(obstacle_list)):
        distobs=distance_points(pn,obstacle_list[i])
        if(distobs<=obstacle_list[i][2]):
            pobs=obstacle_list[i] 
            break 
    """
    ...
    #cv2.circle(image_coy,(pobs[1],pobs[0]),2,(0,0,0),-1)           
    a=180-angle(pobs,snode,dnode) 
    distobs1=distance_points(snode,pobs)

    perpend_dist=distobs1*math.sin((a/180.0)*math.pi)
...
def two_points(obsnode,t_theta):
    node=list()
    angle=0.0      
    node1=list()
    node2=list()
    angle=t_theta+(math.pi/2)  # to decrease angle between points
    node1.append(int(obsnode[0]-((obsnode[2]+int(1*6/scale_factor))*(math.sin(angle)))))
    node1.append(int(obsnode[1]+((obsnode[2]+int(1*6/scale_factor))*(math.cos(angle)))))
    node1.append(obsnode[3])
    node2.append(int(obsnode[0]+((obsnode[2]+int(1*6/scale_factor))*(math.sin(angle)))))
    node2.append(int(obsnode[1]-((obsnode[2]+int(1*6/scale_factor))*(math.cos(angle)))))
...
def no_add_points(sn,dn,obs,t_theta):
    add_points=list()
    points=list()
    rad=obs[2]
    distance_src=distance_points(sn,obs)
    distance_dest=distance_points(dn,obs)
    n=two_points(obs,t_theta)

    increment_ang=15/rad


    ang_src=math.acos(obs[2]/distance_src)
    ang_dest=math.acos(obs[2]/distance_dest)

    for i in range(len(n)):
        z=1
        source_angle=((180-angle(n[i],obs,sn))*math.pi)/180.0
        destination_angle=((180-angle(n[i],obs,dn))*math.pi)/180.0
        ang_f=destination_angle-ang_dest
        ang_b=source_angle-ang_src
        total_angle=ang_b+ang_f
        testpoint=two_points(obs,t_theta+ang_f)[i]

        if (distance_points(testpoint,dn)>distance_dest):
            testpoint=two_points(obs,t_theta-ang_f)[i]
            z=-1

        for j in range (math.ceil(total_angle/increment_ang)):
            points.append(two_points(obs,t_theta+(z*(ang_f-(i*increment_ang)))))
        points.reverse()
...
def process(sn,dn,pn):                                        # form [row][col][alt]
    print "fffff",pn
    obstacle,t_theta1,dist=identify_obstacle(sn,dn,pn)

    add=no_add_points(sn,dn,obstacle,t_theta1)  #  to add angle
...
def best_path(list1,list2):
    ...
    if(count==0):
        ...
        """
    for i in range(len(sumangle_array)):
        sumangle_array[i]=sumangle_array[i]+angle_array[i]
    return list1[sumangle_array.index(min(sumangle_array))]
