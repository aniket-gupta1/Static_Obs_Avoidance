...
def two_points(obsnode,t_theta,a):
    ...
    for i in range(-a,a+1):       
         ...
         node1.append(int(obsnode[0]-((obsnode[2]+int(2*10/scale_factor))*(math.sin(angle)))))
         node1.append(int(obsnode[1]+((obsnode[2]+int(2*10/scale_factor))*(math.cos(angle)))))
         node1.append(obsnode[3])
         node2.append(int(obsnode[0]+((obsnode[2]+int(2*10/scale_factor))*(math.sin(angle)))))
         node2.append(int(obsnode[1]-((obsnode[2]+int(2*10/scale_factor))*(math.cos(angle)))))
...
def no_add_points(obs,dist):
    ...
    return 2          # first is the number of points to be plotted around the obsstacle
...
def triangular(lastnode,startnode,destnode):
    waypointarray=list()
    start_node=list()
    dest_node=list()
    ...
    new_points=process(lastnode,start_node,dest_node)
...
image_rep=image/255                                                       # converts the image to 0 and 1 with "zero" rePresenting obstacles and geofence boundary and one rePresenting safe Path
...
while (i1<(len(waypoint_array)-1)):                                 # wPary 2 is the list of wayPoints in [column][row] format
    starting_node1=list(waypoint_array[i1])                                         # ith node as start node
    destination_node1=list(waypoint_array[i1+1])
                                     # i+1 th node as destination node
    pathlist.append([flor(starting_node1[0]),flor(starting_node1[1])])

    cv2.circle(image_coy,(starting_node1[1],starting_node1[0]),2, (0,0,0), -1)  
    cv2.circle(image_coy,(destination_node1[1],destination_node1[0]),2, (0,0,0), -1)  

    att_gradient=np.zeros((row_s,col_s))                         
    att_gradient=mark_attractive(destination_node1)                          # edtwo is the morPhological function for the attractive function
        # only for visibility of end node


    # print rep_gradient.astype(int)
    # print att_gradient.astype(int)
    ...
    xi = 1/700.0
    attractive=att_gradient*2.5
                # array storing attractive values
    ...
    while ([flor(starting_node1[0]),flor(starting_node1[1])]!=[destination_node1[0],destination_node1[1]]):       # works till current node is not equal to dest node
        c+=1
        z=flor(starting_node1[0])
        w=flor(starting_node1[1])
        ...
        starting_node1[0]=starting_node1[0]+mag[0]                              # normalized values gets added to the current node  ( till now all values are in float)
        starting_node1[1]=starting_node1[1]+mag[1] 
        if((c%25==0)|(c==0)):             # here 20 in n the spacing between waypoints
            node12=[flor(starting_node1[0]),flor(starting_node1[1]),starting_node1[2]]       
            pathlist.append(node12)
            cv2.circle(image_coy,(node12[1],node12[0]),2, (175,175,175), -1)  
            cv2.waitKey(1)
        
        if(array_image[flor(starting_node1[0])][flor(starting_node1[1])]==100):
            del pathlist[-1]
            del pathlist[-1]
            circle_waypoints=triangular(pathlist[-2],pathlist[-1],destination_node1)
            circle_waypoints.insert(0,pathlist[-1])
            for j in range (len(circle_waypoints)):

                waypoint_array.insert(i1+j+1,circle_waypoints[j])
            break    
        cv2.circle(image_coy,(flor(starting_node1[1]),flor(starting_node1[0])), 0, (175,175,175), -1)    # marks the returned node in the image 
        cv2.imshow("imAgE",image_coy)    
    i1=i1+1              
pathlist.append(destination_node1)    
                     # net arra
